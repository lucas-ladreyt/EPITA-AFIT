...
let init_eratosthenes n =
  let rec main m = match m with
      _ when m >= n + one -> []
     |_ when m >= (of_int 3) && erem m (of_int 2) = one -> m::main (m+one)
     |_ when m >= (of_int 3) && erem m (of_int 2) = zero -> main (m+one)
     |_ -> (of_int 2)::main (m+one)
  in main (of_int 2);;
...
let eratosthenes n =
  let rec deldivs list a = match list with
      [] -> []
     |e::l when erem e a = zero -> deldivs l a
     |e::l -> e::(deldivs l a)
  in let rec main a = match a with
         _ when a > n -> []
        |_ when a*a > n -> a::(main (a+one))
        |_ -> a::(deldivs (main (a+one)) a)
     in main (of_int 2);;
...
let double_primes limit isprime =
  let rec main = function
      [] -> []
     |e::l when isprime ((of_int 2)*e+one) -> (e,((of_int 2)*e+one))::main l
     |e::l -> main l
